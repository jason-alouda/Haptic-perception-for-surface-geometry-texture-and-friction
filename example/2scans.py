...
def run_scan():
    ...
    print("Timestamp, Current1, Current2, Current3, Current4, Position1, Position2", file=fout)
    ...
    # Doing a scan of current
    
    # Switching to current control for motor2
    # reader.Set_Value(reader.m2id, 11, 1, 0)
    ...
    # Doing a position scan
    
    fname = "position_scan.csv"
    fout1 = open(fname, "w")
    print("x, y", file = fout1)
    
    # Switching to position control for motor2
    #reader.Set_Value(reader.m2id, 11, 1, 3)
    ...
    for j in range(N_QUERIES):
        ...
        print("%05d, %05d" % (dxl1_current, dxl2_current))
        print("%05d, %05d" % (position1, position2), file = fout1)
        
        if (dxl1_current < -20):
            reader.Set_Value(reader.m2id, ADDR_PRO_GOAL_POSITION, LEN_PRO_GOAL_POSITION, position2 - 6)
            print("Moving up")
        if (dxl1_current > -3 and dxl2_current < -4) :
            reader.Set_Value(reader.m2id, ADDR_PRO_GOAL_POSITION, LEN_PRO_GOAL_POSITION, position2 + 6)    
            print("Moving down")
